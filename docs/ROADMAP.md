...
- [ ] **Graph IR tensor ops**: extend `graph_ir.rs`'s scalar `Op`s to vectors/matrices so tensor expressions get the same CSE/folding/DCE treatment — split out of the item above; the tensor *type* landed first, the IR integration is its own chunk.
